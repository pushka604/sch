...
def remove_from_storage_view():
    item_id = str(input(prompt(t.management.remove_thing_choice)))

    clear_console()

    if delete_storage_item(item_id):
        print(f'{t.management.removed} {item_id} {t.management.from_storage}')
        time.sleep(4)
    else:
        print(f'{item_id} {t.management.is_not_in_storage}')
        time.sleep(4)
